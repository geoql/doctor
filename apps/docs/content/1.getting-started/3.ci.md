...
The published [`geoql/doctor-action`](https://github.com/marketplace/actions/doctor-vue-nuxt-code-audit)
wraps the above patterns with sensible defaults and works for both
frameworks via the `framework` input:
...
The action installs the right CLI, runs the audit, emits SARIF, and
returns the same exit codes as the raw CLI.

## Push to the dashboard

Add an `api-key` and the action streams **privacy-stripped** findings
to your [the-doctor.report](https://app.the-doctor.report) dashboard —
the score, severity counts, and per-rule findings, charted over time.
...
- uses: geoql/doctor-action@v2
  with:
    ...
    project: ${{ github.repository }} # the dashboard slug, owner/repo
```

Only privacy-safe fields leave your CI: rule id, severity, category,
file path, and line/column. **Never** source code, snippets, or
messages. A failed push never changes the audit exit code.

::callout{type="tip"}
On the raw CLI the equivalent flags are `--push`, `--api-key`, and
`--push-project`. Note `--push-project` (the dashboard slug) is
distinct from `--project` (the monorepo workspace filter).
::

### Monorepos: one job per sub-app

In a repo that ships more than one app, run the action once per sub-app
with a different `working-directory`. The action forwards it as the
workspace label, so each sub-app keeps its own score series, findings,
and leaderboard row under a single project slug.
...
Without distinct workspaces both jobs push to the same untagged series,
and runs sharing a commit collapse to the newest — one app's score
silently replaces the other's. On the raw CLI the flag is
`--push-workspace <path>`.
