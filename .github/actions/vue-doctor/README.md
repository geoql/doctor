...
A composite GitHub Action that audits a Vue project with [`@geoql/vue-doctor`](https://www.npmjs.com/package/@geoql/vue-doctor). It reports a quality score, a findings count, and writes a SARIF report you can upload to GitHub Code Scanning.

The action installs `vue-doctor` globally with pnpm and runs the audit once to produce JSON (which drives the outputs and the pass/fail gate), then emits a SARIF report from the same arguments.
...
jobs:
  audit:
    ...
    steps:
      ...
      - name: Run Vue Doctor
        id: doctor
        uses: geoql/doctor/.github/actions/vue-doctor@main
        with:
          preset: recommended
          threshold: '80'
          fail-on: error
...
| `working-directory` | Directory to run the audit in (the Vue project root).                          | `.`           | No       |
...
- The job fails when `vue-doctor` exits non-zero, which is governed by `fail-on` (severity gate) and `threshold` (score gate).
