...
const PLUGIN_REL = 'node_modules/@geoql/oxlint-plugin-vue-doctor';

async function tmp(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'geoql-doctor-resolve-'));
}

async function writePluginPkg(
  root: string,
  pkg: Record<string, unknown>,
): Promise<void> {
  const pkgDir = join(root, PLUGIN_REL);
...
describe('resolveVueDoctorPluginPath upward lookup + readPkgMain', () => {
  it('resolves via exports["."].import', async () => {
    ...
    await writePluginPkg(root, { exports: { '.': { import: './dist/x.js' } } });
    const resolved = resolveVueDoctorPluginPath(root);
    expect(resolved).toBe(join(root, PLUGIN_REL, 'dist/x.js'));
  });

  it('resolves via exports["."].default when no import', async () => {
    const root = await tmp();
    await writePluginPkg(root, {
      exports: { '.': { default: './dist/def.js' } },
    });
    expect(resolveVueDoctorPluginPath(root)).toBe(
      join(root, PLUGIN_REL, 'dist/def.js'),
    );
  });

  it('resolves via top-level module field', async () => {
    const root = await tmp();
    await writePluginPkg(root, { module: './dist/mod.js' });
    expect(resolveVueDoctorPluginPath(root)).toBe(
      join(root, PLUGIN_REL, 'dist/mod.js'),
    );
  });

  it('resolves via top-level main field', async () => {
    const root = await tmp();
    await writePluginPkg(root, { main: './dist/main.js' });
    expect(resolveVueDoctorPluginPath(root)).toBe(
      join(root, PLUGIN_REL, 'dist/main.js'),
    );
  });

  it('falls back to ./dist/index.js when package.json has no entry fields', async () => {
    const root = await tmp();
    await writePluginPkg(root, { name: '@geoql/oxlint-plugin-vue-doctor' });
    expect(resolveVueDoctorPluginPath(root)).toBe(
      join(root, PLUGIN_REL, 'dist/index.js'),
    );
  });
