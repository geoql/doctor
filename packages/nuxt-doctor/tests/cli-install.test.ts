...
const SKILL_REL = join('.agents', 'skills', 'nuxt-doctor', 'SKILL.md');

describe('nuxt-doctor install', () => {
  let stdout: string[];
  let stderr: string[];
  let dir: string;
  ...
  beforeEach(() => {
    stdout = [];
    stderr = [];
    dir = mkdtempSync(join(tmpdir(), 'nuxt-install-'));
  ...
  it('writes nothing under --dry-run but prints the target path + content', async () => {
    ...
    expect(out).toContain('name: nuxt-doctor');
  ...
  it('writes the SKILL.md with correct frontmatter + doctor command under --yes', async () => {
    const { run } = await import('../src/cli.js');
    const code = await run(['node', 'nuxt-doctor', 'install', '--yes', dir]);
    expect(code).toBe(0);
    const written = readFileSync(join(dir, SKILL_REL), 'utf-8');
    expect(written).toContain('name: nuxt-doctor');
    expect(written).toContain('Use when finishing a feature');
    expect(written).toContain('before committing Nuxt');
    expect(written).toMatch(/version:/);
    expect(written).toContain('npx @geoql/nuxt-doctor@latest --diff --verbose');
    expect(written).toContain('npx @geoql/nuxt-doctor@latest');
    expect(written).toContain(
      'https://docs.the-doctor.report/prompts/doctor-agent.md',
    );
    expect(written).toContain(
      'https://docs.the-doctor.report/prompts/rules/nuxt-doctor/',
    );
  ...
  it('is a no-op with a clear message when the skill exists without --force', async () => {
    const { run } = await import('../src/cli.js');
    await run(['node', 'nuxt-doctor', 'install', '--yes', dir]);
  ...
  it('writes after an interactive confirm', async () => {
    ...
    const { run } = await import('../src/cli.js');
    const code = await run(['node', 'nuxt-doctor', 'install', dir]);
    expect(code).toBe(0);
    expect(existsSync(join(dir, SKILL_REL))).toBe(true);
  });
  ...
  it('defaults to the current working directory when no [dir] is provided', async () => {
    ...
    try {
      const { run } = await import('../src/cli.js');
      const code = await run(['node', 'nuxt-doctor', 'install', '--yes']);
      expect(code).toBe(0);
      expect(existsSync(join(dir, SKILL_REL))).toBe(true);
    } finally {
