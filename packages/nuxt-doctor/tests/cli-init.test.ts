...
describe('nuxt-doctor init + --pr-comment', () => {
  let stdout: string[];
  let stderr: string[];
  let dir: string;
  ...
  beforeEach(() => {
    stdout = [];
    stderr = [];
    dir = mkdtempSync(join(tmpdir(), 'nuxt-init-'));
  ...
  it('writes a doctor.config.ts with recommended defaults under --yes', async () => {
    ...
    const code = await run(['node', 'nuxt-doctor', 'init', '--yes', dir]);
    expect(code).toBe(0);
    ...
    expect(pkg.scripts['doctor:check']).toBe('nuxt-doctor');
  ...
  it('exits 2 when prompts throws an Error', async () => {
    ...
    const { run } = await import('../src/cli.js');
    const code = await run(['node', 'nuxt-doctor', 'init', dir]);
    expect(code).toBe(2);
  ...
  it('interactive path with package-json target and exclude list', async () => {
    mockPromptAdapter.default.mockResolvedValueOnce({
      target: 'package-json',
      preset: 'minimal',
      threshold: 0,
