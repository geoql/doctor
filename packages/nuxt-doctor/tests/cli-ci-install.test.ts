...
describe('nuxt-doctor ci install', () => {
  let stdout: string[];
  let stderr: string[];
  let dir: string;
  ...
  beforeEach(() => {
    stdout = [];
    stderr = [];
    dir = mkdtempSync(join(tmpdir(), 'nuxt-ci-install-'));
  ...
  it('writes the GitHub workflow under --yes', async () => {
    ...
    expect(written).toContain('framework: nuxt');
  ...
  it('writes .gitlab-ci.yml with --provider gitlab', async () => {
    ...
    expect(written).toContain('npx -y @geoql/nuxt-doctor@latest');
    expect(existsSync(join(dir, WORKFLOW_REL))).toBe(false);
  });
  ...
  it('rejects unknown ci actions like upgrade with exit 2', async () => {
    const { run } = await import('../src/cli.js');
    const code = await run(['node', 'nuxt-doctor', 'ci', 'upgrade', dir]);
    expect(code).toBe(2);
  ...
  it('rejects ci config with exit 2', async () => {
    const { run } = await import('../src/cli.js');
    const code = await run(['node', 'nuxt-doctor', 'ci', 'config', dir]);
    expect(code).toBe(2);
  ...
  it('defaults to the current working directory when no [dir] is given', async () => {
    ...
    try {
      const { run } = await import('../src/cli.js');
      const code = await run(['node', 'nuxt-doctor', 'ci', 'install', '--yes']);
      expect(code).toBe(0);
      expect(existsSync(join(dir, WORKFLOW_REL))).toBe(true);
    } finally {
