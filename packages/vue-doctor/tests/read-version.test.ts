...
describe('readVersion fallback', () => {
  ...
  it('still audits when reading package.json throws (catch branch)', async () => {
    ...
    vi.doMock('node:fs', async (importOriginal) => {
      ...
      return {
        ...actual,
        // readVersion() is the only sync package.json read; the template
        // pass reads SFCs via fs/promises.readFile, so forcing this to throw
        // exercises readVersion()'s catch without breaking the audit.
        readFileSync: (
  ...
  it('defaults version to 0.0.0 when package.json has no version field', async () => {
    ...
    vi.doMock('node:fs', async (importOriginal) => {
      ...
      return {
        ...actual,
        // A package.json without a `version` key exercises the
        // `pkg.version ?? '0.0.0'` nullish branch in readVersion().
        readFileSync: (
          path: Parameters<typeof actual.readFileSync>[0],
          ...rest: unknown[]
        ) => {
