...
describe('vue-doctor --fix', () => {
  ...
  it('applies the oxlint fix to disk and prints a summary line', async () => {
    ...
    try {
      ...
      await run(['node', 'vue-doctor', dir, '--no-dead-code', '--fix']);
  ...
  it('warns about --fix-exclude not being enforced for built-in rules', async () => {
  ...
  it('reports singular finding correctly in summary line', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'vue-cli-fix-singular-'));
  ...
  it('reports plural findings correctly in summary line', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'vue-cli-fix-plural-'));
    const file = join(dir, 'Comp.vue');
    const file2 = join(dir, 'Comp2.vue');
    writeFileSync(
      file,
    ...
    try {
      await run(['node', 'vue-doctor', dir, '--no-dead-code', '--fix']);
      expect(stderr.join('')).toMatch(/findings remain/);
    } finally {
      rmSync(dir, { recursive: true, force: true });
    }
  }, 60_000);
